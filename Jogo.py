...
class Jogo:
    def __init__(self, nick, credito=0, progresso=0, nivel=1, xp=0, inventario=None, conquistas=None):
        ...
        self.DB_CONQUISTAS = {
            "Recruta": "Completar a primeira missão",
            "Mãos de Plástico": "Comprar Luvas de Boxe",
            "Sortudo": "Ganhar no jogo de Dados",
            "Magnata": "Acumular R$5.000 em saldo",
            "Veterano": "Chegar ao nível 5",
            "Cibercriminoso": "Desbloquear o computador na Missão 3"
        }
    ...
    def experiencia(self, exp):
        ...
        while True:
            ...
            if self._xp >= prox_nivel:
                ...
                self._xp -= prox_nivel
                self.exibir_msg(f"✨ LEVEL UP! Nível: {self._nivel} | Bônus: +R${recompensa}", 'verde', 2)
            else:
                break
    ...
    def verificar_conquistas(self):
        regras = {
            ...
            "Mãos de Plástico": "Luvas de boxe" in self.inventario,
        ...
        for nome, condicao in regras.items():
            if condicao and nome not in self.conquistas:
                ...
                print(f"{CORES['roxo']}🏆 CONQUISTA DESBLOQUEADA: {nome}!{CORES['limpar']}")
                self.salvar()
    ...
    def tela_conquistas(self):
        ...
        for nome, desc in self.DB_CONQUISTAS.items():
            status = f"{CORES['verde']}⭐ [LIBERADA]" if nome in self.conquistas else f"{CORES['vermelho']}🔒 [BLOQUEADA]"
    ...
    def missoes(self):
        ...
        if self._progresso_atual < len(lista):
            ...
            self.exibir_msg("🎉 Todas as missões concluídas!", 'amarelo')

    def missao1(self):
        self.limpar_tela()
        print("--- MISSÃO 1: O HACKER INICIANTE ---")
    ...
    def missao2(self):
        self.limpar_tela()
        print("--- MISSÃO 2: DUELO DE REFLEXOS ---")
        vitorias = 0
        while vitorias < 3:
        ...
        while True:
            ...
            if item_sel:
                if self._credito >= vitrine[item_sel]:
                    ...
                    self.exibir_msg(f"✅ {item_sel} comprado!", 'verde')
                    self.verificar_conquistas()
    ...
    def cassino(self):
        while True:
            self.limpar_tela()
            print(f"--- CASSINO ---")
            print("1. Dados | 2. Caça-Niquel | 3. Voltar")
            op = input("Escolha: ")
            if op == "1": self.dados()
    ...
    def caca_niquel(self):
        ...
        for simbolo in config_simbolos:
            ...
            if quantidade >= 3:
                ...
                msg_extra = f" (+{extras} bônus!)" if extras > 0 else ""
                print(f"✨ VITÓRIA! {quantidade}x {simbolo}{msg_extra}! +R${premio}")

        if venceu:
            bonus_cash, _ = self.bonus_item(ganho_total, 0)
            self._credito += (ganho_total + bonus_cash)
            self.experiencia(5 + maior_extra)
            print(f"\n💰 TOTAL RECEBIDO: R${ganho_total + bonus_cash}")
        else:
            print("\n❌ Tente novamente!")
    ...
    def gameplay(self):
        while True:
            ...
            print(f"👤 {self._nick} | ⭐ Lvl: {self._nivel} | 💰 R${self._credito}")
            print(f"🏆 Conquistas: {len(self.conquistas)}/6")
