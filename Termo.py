...
class Termo:
    ...
    def animar_vitoria(self):
        """Cria um efeito de carregamento antes de revelar a vitória."""
        mensagens = ["VALIDANDO", "ANALISANDO", "VERIFICANDO"]
        print("\n")
        for msg in mensagens:
            for i in range(4):
                # O \r faz o cursor voltar para o início da linha
    ...
    def adivinhar(self):
        ...
        while len(historico) < max_tentativas:
            ...
            if len(chute) != len(self.palavra):
                self.limpar_tela()
                print(f"⚠️ Erro: Digite {len(self.palavra)} letras!")
                continue
            ...
            if chute == self.palavra:
                self.animar_vitoria() # Aciona a animação aqui!
                self.limpar_tela()
                print(" --- 🟩 JOGO DO TERMO 🟨 --- ")
                for h in historico: print(h)
                print(f"\n🎉 Incrível! Você acertou em {len(historico)} tentativas!")
                return
